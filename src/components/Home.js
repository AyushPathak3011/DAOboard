...
export const Home = () => {
  ...
  const handleHover = (id) => {
    document.getElementById(id).className =
      "bg-slate-300 p-1 rounded-xl cursor-pointer hover:scale-105 transition-all ease-in-out hover:cursor-text hover:z-10";
  ...
  const mouseLeaveHandler = (id) => {
    document
      .getElementById(id)
      .addEventListener("mouseleave", () => setLimit({ id: id, lim: 32 }));
  };
  ...
  return (
    <div>
        ...
        <Box sx={style}>
          <div className="flex flex-col  justify-center gap-4">
            ...
            <div className="flex justify-between">
              <button
                onClick={() => registerForTask(taskId, comment)}
              ...
              <button
                onClick={() => openCloseVolList(taskId)}
                className="flex justify-center items-center cursor-pointer"
              >
                {" "}
                <p className="text-xs font-semibold">View Volunteers</p>{" "}
                {handleVolList === true ? (
                  <ArrowDownwardIcon />
                ) : (
                  <DoubleArrowIcon />
            ...
            {handleVolList === true && (
              <div className="vollist mt-4 overflow-y-scroll overflow-x-hidden max-h-44 bg-blue-400 p-5 rounded-lg flex flex-col ">
                {loader1 === false ? (
                  volList.map((dat) => (
                    <div className="flex justify-between items-center mt-4 ">
                      <p>{dat[0]}</p>
                      {currentUser.toLowerCase() === dat[1].toLowerCase() ? (
                        <img
                          onClick={() =>
                            navigate("/taskstatus/" + dat[1] + "/" + taskId)
                          }
                          className="w-8 animate-spin cursor-pointer transition-all ease-in-out hover:scale-125"
                          src={daologo}
                        />
                      ) : (
                        ""
                      ...
                      <p
                        id={dat[1] + "xyz"}
                        onMouseLeave={() => mouseLeaveHandler(dat[1] + "xyz")}
                        onMouseEnter={() => handleHover(dat[1] + "xyz")}
                        className="bg-slate-300 p-1 rounded-xl cursor-pointer transition-all ease-in-out"
                      >
                        {dat[1].slice(
                          0,
                          limit.id === dat[1] + "xyz" ? limit.lim : 32
                        )}
                      </p>
                    </div>
                  ))
        ...
        <div className="flex gap-4">
          <div className="connectwallet">
            <button
              ...
              className={
                providerConnected
                  ? "w-fit p-2 bg-slate-500 rounded-xl  border-2 border-blue-200"
                  : "active:bg-slate-400 w-fit p-2 bg-slate-500 rounded-xl  border-2 border-blue-200 transition-all ease-in-out hover:scale-105"
              }
            >
              {providerConnected ? "Connected" : "Connect Wallet"}
            </button>
          </div>
          ...
          <div className="admin self-end">
            {isadmin ? (
              <button
                ...
                className="w-fit p-2 bg-slate-500 rounded-xl active:bg-slate-400 border-2 border-blue-200 transition-all ease-in-out hover:scale-105"
              >
                Admin Console
              </button>
            ) : (
      ...
      <div className="hero h-screen flex flex-col items-center">
        ...
        <div className="button m-auto flex gap-10 flex-wrap">
          <div className="viewldboard">
            <Link to="/leaderboard">
              <button className="w-fit px-6 py-4 bg-slate-500 rounded-xl border-2 border-blue-200 active:bg-slate-400 hover:bg-sky-100 transition-all ease-in-out hover:scale-105">
                View Leaderboard
              </button>
            </Link>
          </div>
        ...
        <div className="faucet mb-56">
          <p className="text-slate-300 font-semibold">
            Mumbai Faucet -{" "}
            <a
              className="px-1.5 py-1.5 text-black bg-slate-400 rounded-xl border-2 border-blue-200 active:bg-slate-300  hover:bg-sky-100 transition-all ease-in-out hover:scale-105"
      ...
      <section className="tasksMain -mt-28 flex flex-col justify-center items-center px-6">
        <p className="font-bold text-blue-100 text-2xl animate-pulse shadow-lg p-4 mb-10">
          COMMUNITY TASKS
        </p>
        <div className="grid grid-cols-2 min-h-[300px] mx-4 mb-12 w-full">
          <div className="flex flex-col bg-sky-800 items-center mx-10 shadow-2xl border-t-2 rounded-xl">
            <p className="font-bold text-blue-100 text-xl mt-2">
              Ongoing Tasks
            </p>
                  ...
                  .map((dat) => (
                    <div className="taskItem flex bg-sky-700 flex-col w-4/5 justify-center my-4 shadow-lg border-b-4 border-l-2 border-blue-300 rounded-3xl px-4 py-2 transition-all ease-in-out hover:bg-blue-50 hover:opacity-75">
                      <div className="flex my-2 items-center justify-between">
                        <p className="taskTitle font-extrabold  bg-sky-100 w-fit rounded-xl p-2">
                          {dat[1]}
                        </p>
                      ...
                      <p className="taskDesc font-semibold mx-2">
                        {dat[2].slice(0, 100) + "...."}
                      </p>
                      <div className="flex justify-between mx-4 items-center">
                        <button
                          ...
                          className="w-fit px-2 m-4 bg-blue-200 rounded-xl active:bg-slate-400 border-2 border-blue-200 transition-all ease-in-out hover:scale-105 font-semibold"
                        >
                          Register
                        </button>
                        <div>
              ...
              {taskLoader === false ? (
                tasks
                  ...
                  .map((dat) => (
                    <div className="taskItem flex bg-sky-700 flex-col w-4/5 justify-center my-4 shadow-lg border-b-4 border-l-2 border-blue-300 rounded-3xl px-4 py-2 transition-all ease-in-out hover:bg-blue-50 hover:opacity-75">
                      <div className="flex my-2 items-center justify-between">
                        <p className="taskTitle flex gap-4 font-extrabold  bg-red-400 w-fit rounded-xl p-2">
                          {dat[1]} <p className="text-red-900">CLOSED</p>{" "}
                        </p>
                        <p className="font-semibold mx-2">
                          🪙: {parseInt(dat[3]._hex)}
                        </p>
